/**
 * PDFDropdown — native COS form field.
 */
...
import { generateDropdownAppearance } from './FieldAppearanceGenerator.js';
...
export class PDFDropdown extends PDFField {
  ...
  /** @internal */
  static _createNative(info: NativeFieldInfo, ctx: NativeDocumentContext): PDFDropdown {
    return new PDFDropdown(info, ctx);
  }
  ...
  select(options: string | string[], _merge?: boolean): void {
    ...
    try {
      generateDropdownAppearance(this._nativeCtx!, this._native!.dict, val);
    } catch {
  ...
  addToPage(page: PDFPage, _options?: FieldAppearanceOptions): void {
    ...
    throw new Error('PDFDropdown.addToPage() requires native mode pages.');
  }
