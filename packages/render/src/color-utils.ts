 ...
 *
 * Provides RGB/HSL conversions, hex parsing, alpha compositing, and
 * CSS color string formatting. These utilities are independent of any
 * XML parsing or theme resolution — they operate only on plain color values.
 *
...
/** RGBA color with components 0-255 for r/g/b and 0-1 for alpha. */
...
export function compositeOver(fg: RgbaColor, bg: RgbaColor): RgbaColor {
  ...
  return { r: clampByte(Math.round(r)), g: clampByte(Math.round(g)), b: clampByte(Math.round(b)), a };
}
