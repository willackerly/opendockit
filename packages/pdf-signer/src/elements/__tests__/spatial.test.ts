...
import { baseProps, makeTextElement, makeShapeElement, makeImageElement } from './test-helpers.js';
...
describe('pointInRect', () => {
  ...
  it('returns false for point outside', () => {
    expect(pointInRect(5, 5, rect)).toBe(false);
    expect(pointInRect(35, 35, rect)).toBe(false);
  });

  it('returns true for point on edge (inclusive)', () => {
    expect(pointInRect(10, 10, rect)).toBe(true);   // top-left corner
    expect(pointInRect(30, 30, rect)).toBe(true);   // bottom-right corner
    expect(pointInRect(20, 10, rect)).toBe(true);   // top edge
    expect(pointInRect(10, 20, rect)).toBe(true);   // left edge
  });

  it('returns false for point just outside edge', () => {
    expect(pointInRect(9.999, 15, rect)).toBe(false);
    expect(pointInRect(30.001, 15, rect)).toBe(false);
  });
});
...
describe('elementAtPoint', () => {
  ...
  it('returns the topmost (last) element at the point', () => {
    ...
    const result = elementAtPoint(elements, 75, 75);
  ...
  it('returns the only element at a point', () => {
    ...
    const result = elementAtPoint(elements, 10, 10);
    expect(result).not.toBeNull();
    expect(result!.id).toBe('s1');
  });

  it('returns null for a miss', () => {
    const result = elementAtPoint(elements, 300, 300);
    expect(result).toBeNull();
  });

  it('returns null for empty element list', () => {
    const result = elementAtPoint([], 10, 10);
    expect(result).toBeNull();
  });
});
