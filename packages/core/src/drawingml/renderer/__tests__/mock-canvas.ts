/**
 * Mock Canvas2D context and RenderContext for renderer unit tests.
 *
 * Records all method calls so tests can assert the correct Canvas2D API
 * sequence without needing a real browser canvas.
 */

import type { ThemeIR, ResolvedColor } from '../../../ir/index.js';
import { MediaCache } from '../../../media/index.js';
...
export function createMockContext(): MockContext {
  ...
  const ctx = {
    ...
    measureText: (text: string) => {
      // Extract font size from ctx.font (e.g. "18pt sans-serif" -> 18pt, "10px sans-serif" -> 10px).
...
/** Create a real MediaCache instance for testing. */
function createTestMediaCache(): MediaCache {
 ...
 *
 * @param ctxOverride - Optional pre-created mock context. If omitted, a new
 *                      one is created via {@link createMockContext}.
